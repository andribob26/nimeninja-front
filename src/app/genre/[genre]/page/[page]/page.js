...
const AnimeGenrePage = async ({ params }) => {
  ...
  try {
    const resAnimeGenre = await fetchWithRevalidate(
      "/media",
      {
        page: pageNumber,
        genre: genreName,
    ...
    animes = resAnimeGenre.data;
    pagination = resAnimeGenre.pagination;
  } catch (err) {
    console.error(`Anime genre failed to fetch:`, err);
    return notFound();
  }
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "@type": "CollectionPage",
            name: `Anime Genre ${genreName} Sub Indo`,
            description: `Kumpulan anime genre ${genreName} subtitle Indonesia terbaru di NimeNinja.`,
            url: `${process.env.NEXT_PUBLIC_SITE_URL}/genre/${genreName}`,
          }),
        }}
      />
      <section
        aria-labelledby={`anime-${genre}`}
              ...
              {pagination.page > 1 && (
                <Link
                  href={
                    pagination.page === 2
                      ? `/anime/status/${status}`
                      : `/anime/status/${status}/page/${pagination.page - 1}`
                  }
                  className="px-2 py-1 md:px-3 md:py-2 flex items-center justify-center bg-gray-700 hover:bg-gray-600"
                >
